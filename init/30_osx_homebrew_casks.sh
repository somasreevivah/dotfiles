...
[[ ! "$(brew ls --versions brew-cask)" ]] && dfs_error "Brew-cask failed to install." && return 1
...
if (( ${#casks[@]} > 0 )); then
  dfs_header "Installing Homebrew casks: ${casks[*]}"
...
if (( ${#cps[@]} > 0 )); then
  dfs_header "Fixing colorPicker symlinks"
  for f in "${cps[@]}"; do
    target="$(readlink "$f")"
    dfs_arrow "$(basename "$f")"
    rm "$f"
